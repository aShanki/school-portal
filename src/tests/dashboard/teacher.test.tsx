...
describe('Teacher Dashboard', () => {
  ...
  it('handles unauthenticated access', async () => {
    ...
    await renderWithProviders(<TeacherDashboard />, {
      session: null // Pass null session to provider
    })
  ...
  it('displays teacher stats correctly', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(await screen.findByTestId('dashboard-title')).toHaveTextContent('Teacher Dashboard')
...
describe('Teacher Classes Page', () => {
  ...
  it('displays class list correctly', async () => {
    ...
    await waitFor(() => {
      expect(mockRouter.push).toHaveBeenCalledWith('/dashboard/teacher/classes/1')
    })
  })
})
...
describe('Teacher Attendance Page', () => {
  ...
  it('displays attendance overview correctly', async () => {
    ...
    await waitFor(async () => {
      expect(await screen.findByRole('heading', { name: /attendance overview/i })).toBeInTheDocument()
      expect(await screen.findByText('Math 101')).toBeInTheDocument()
      expect(await screen.findByText('Mathematics')).toBeInTheDocument()
      expect(await screen.findByRole('button', { name: /take attendance/i })).toBeInTheDocument()
    })
  })

  it('handles API error states', async () => {
    ;(global.fetch as jest.Mock) = jest.fn().mockRejectedValue(new Error('Failed to fetch'))
    ;(useSession as jest.Mock).mockReturnValue({
      data: { user: { role: 'TEACHER' } },
      status: 'authenticated'
    })

...
import { Pencil, Check } from 'lucide-react' // Add this import
...
export default function ClassDetailsPage({
  ...
  const { data: classData, isLoading } = useQuery({
    queryKey: ['class', id],
    queryFn: async () => {
      const res = await fetch(`/api/dashboard/teacher/classes/${id}`)
      if (!res.ok) throw new Error('Failed to fetch class')
      const data = await res.json()
      
      // Add detailed response logging
      console.log('Raw API Response:', data)
      console.log('Grades array type:', Array.isArray(data.grades))
      console.log('Grades array length:', data.grades?.length)
      console.log('First grade if exists:', data.grades?.[0])
  ...
  const gradeMutation = useMutation({
    mutationFn: async ({ studentId, assignmentId, points }: { 
      ...
      const res = await fetch(`/api/dashboard/teacher/classes/${id}/grades`, {
        method: 'POST',
        headers: {
    ...
    onMutate: async ({ studentId, assignmentId, points }) => {
      ...
      await queryClient.cancelQueries(['class', id])

      // Snapshot the previous value
      const previousData = queryClient.getQueryData(['class', id])

      // Optimistically update the grade
      queryClient.setQueryData(['class', id], (old: any) => ({
        ...old,
    ...
    onError: (err, variables, context) => {
      ...
      queryClient.setQueryData(['class', id], context?.previousData)
      toast({
        variant: "destructive",
